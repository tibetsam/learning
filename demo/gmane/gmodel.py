...
def fixbody(body):
    ...
    pos=body.find('-----Original Message-----')         
    body=body[:pos]
    body=body.split('\n')
    text=[]
    for line in body:
        if not line.startswith('>'):text.append(line)
    body='\n'.join(text)
...
for message_row in cur_2 :
    ...
    cur.execute('INSERT OR IGNORE INTO Messages (guid,sender_id,subject_id,sent_at,headers,body) VALUES ( ?,?,?,datetime(?),?,? )', 
            ( guid, sender_id, subject_id, sent_at, zlib.compress(message_row[0]), zlib.compress(body)) )
    conn.commit()
